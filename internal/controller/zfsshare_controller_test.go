...
// TestZfsShareReconcile_LocalNVMeoFSkipsNetworkExport covers ADR-0031: when the
// share's single attached node is also the pool's own current node, no
// NetworkExport is rendered at all, and the share is Bound immediately.
func TestZfsShareReconcile_LocalNVMeoFSkipsNetworkExport(t *testing.T) {
	scheme := newTestScheme(t)
	...
	share := &storagev1alpha1.ZfsShare{
		...
		Spec: storagev1alpha1.ZfsShareSpec{
			...
			Dataset:      "k8s/pvc-2",
			Protocol:     storagev1alpha1.ProtocolNVMeoF,
			NVMeoF:       &storagev1alpha1.NVMeoFExportSpec{AllowedHosts: []string{"nqn.host"}},
			AttachedNode: "node-a",
		},
	}
	...
	r := &ZfsShareReconciler{Client: c, Scheme: scheme}
	if _, err := r.Reconcile(context.Background(), ctrl.Request{NamespacedName: types.NamespacedName{Name: "pvc-2"}}); err != nil {
		t.Fatalf("reconcile: %v", err)
	}

	var export storagev1alpha1.NetworkExport
	if err := c.Get(context.Background(), client.ObjectKey{Name: "pvc-2"}, &export); err == nil {
		t.Fatalf("expected no NetworkExport for a local attach")
	}

	var got storagev1alpha1.ZfsShare
	if err := c.Get(context.Background(), client.ObjectKey{Name: "pvc-2"}, &got); err != nil {
		t.Fatalf("get share: %v", err)
	}
	if got.Status.Phase != storagev1alpha1.SharePhaseBound {
		t.Errorf("phase = %q, want Bound immediately for a local attach", got.Status.Phase)
	}
	if got.Status.NetworkExportName != "" {
		t.Errorf("networkExportName = %q, want empty (no export rendered)", got.Status.NetworkExportName)
	}
}

// TestZfsShareReconcile_PoolMovesAwayFromAttachedNode covers the transition:
// a share that was local-only must fall back to a real NetworkExport once the
// pool moves off the attached node (the existing ZfsPool watch re-drives this).
func TestZfsShareReconcile_PoolMovesAwayFromAttachedNode(t *testing.T) {
	scheme := newTestScheme(t)
	...
	c := fake.NewClientBuilder().
		WithScheme(scheme).
		WithObjects(pool, share).
		WithStatusSubresource(&storagev1alpha1.ZfsShare{}, &storagev1alpha1.NetworkExport{}, &storagev1alpha1.ZfsPool{}).
		Build()

	r := &ZfsShareReconciler{Client: c, Scheme: scheme}
	if _, err := r.Reconcile(context.Background(), ctrl.Request{NamespacedName: types.NamespacedName{Name: "pvc-3"}}); err != nil {
		t.Fatalf("reconcile (local): %v", err)
	}

	// The pool fails over to a different node than the one still attached.
	var movedPool storagev1alpha1.ZfsPool
	if err := c.Get(context.Background(), client.ObjectKey{Name: "zpool-999"}, &movedPool); err != nil {
		t.Fatalf("get pool: %v", err)
	}
	movedPool.Status.CurrentNode = "node-b"
	if err := c.Status().Update(context.Background(), &movedPool); err != nil {
		t.Fatalf("update pool: %v", err)
	}

	if _, err := r.Reconcile(context.Background(), ctrl.Request{NamespacedName: types.NamespacedName{Name: "pvc-3"}}); err != nil {
		t.Fatalf("reconcile (after pool move): %v", err)
	}

	var export storagev1alpha1.NetworkExport
	if err := c.Get(context.Background(), client.ObjectKey{Name: "pvc-3"}, &export); err != nil {
		t.Fatalf("expected a NetworkExport once the pool is no longer local: %v", err)
	}
	if export.Spec.NodeName != "node-b" {
		t.Errorf("export NodeName = %q, want node-b", export.Spec.NodeName)
	}
}
